            ...
            for file_name, df in dataframes.items():
                ...
                for col in df.columns:
                    if col != col.lower() or ' ' in col:
                        return JsonResponse({
                            'status': 'error',
                            'message': f"Column {col} in file {file_name} is not standardized!"
                        })

            # Output DataFrames as JSON for debugging before feature engineering
            df_json_outputs = {
                file_name: df.head(5).to_dict(orient='records')  # Convert first 5 rows of each DataFrame to JSON
                for file_name, df in dataframes.items()
            }

            # Returning file paths and DataFrames preview (head of first 5 rows)
            return JsonResponse({
                'status': 'success',
                'message': 'DataFrames loaded successfully. Returning data for review.',
                'file_paths': file_paths,  # Including the file paths in the response
                'dataframes': df_json_outputs
            })

            # Feature engineering (commented to be executed after review of JSON output)
...
def make_predictions(new_data_path, output_path):
    try:
        ...
        preprocessor = joblib.load(os.path.join(MODEL_DIR, 'scaler.pkl'))
